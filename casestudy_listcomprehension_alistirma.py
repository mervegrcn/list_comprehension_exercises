...
columns_list = ["NUM_" + col.upper() if df[col].dtype = "O" else col.upper() for col in df.columns]
print(columns_list)

# Alternatif
columns_list = ["NUM" + col.upper() if df[col].dtype in ["float64", "int64"] else col.upper() for col in df.columns]
print(columns_list)
...
column_list = [col.upper() if "no" in col else col.upper() + "_FLAG" for col in df.columns]
print(column_list)
